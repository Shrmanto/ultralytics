...
class PartialConv2d(nn.Module):
    """Partial Convolution: only applies conv to a subset of input channels."""
    def __init__(self, in_channels, out_channels, kernel_size=3, ratio=0.25, stride=1, padding=1, bias=False):
        super(PartialConv2d, self).__init__()
        self.part = int(in_channels * ratio)
        self.conv = nn.Conv2d(self.part, out_channels, kernel_size=kernel_size,
    ...
    def forward(self, x):
        ...
        if out.shape[1] < x.shape[1]:
            pad = torch.zeros(x.size(0), x.shape[1] - out.shape[1], out.shape[2], out.shape[3], device=x.device)
    ...
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True,mode=None,args=None):
        ...
        if self.mode in ['original']:
            ...
            self.cheap_operation = nn.Sequential(
                PartialConv2d(init_channels, new_channels, kernel_size=dw_size, stride=1, padding=dw_size // 2, ratio=0.25),
